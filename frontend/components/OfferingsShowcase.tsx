...
const offerings = [
    {
        id: 'trainings',
        title: 'Treinamentos',
        subtitle: 'Capacitação & Certificação',
        icon: GraduationCap,
        color: '#FFB000', // Amber/Copper
        gradient: 'from-amber-500 to-orange-500',
        badge: 'Academy',
        items: [
            { title: 'Palestra: Governança de IA e o PL 2338', icon: BookOpen },
            { title: 'Workshop de 8 Horas', icon: Users },
            { title: 'Curso de Extensão (Pós-Graduação)', icon: Award },
            { title: 'Certificação Advisor/Auditor', icon: Shield },
        ],
        footer: null
    },
    {
        id: 'consultancy',
        title: 'Consultorias',
        subtitle: 'Estratégia & Implementação',
        icon: Search,
        color: '#00A3FF', // Blue
        gradient: 'from-blue-500 to-purple-500',
        badge: 'Advisory',
        items: [
            { title: 'Diagnóstico de IA', icon: Search },
            { title: 'Governança: Descoberta', icon: LineChart },
            { title: 'Governança: Gestão', icon: Shield },
            { title: 'Cultura "IA First"', icon: Users },
        ],
        footer: { icon: Users, text: 'Equipe Global: 250+ Membros' }
    },
    {
        id: 'saas',
        title: 'Sistemas SaaS',
        ...
        featured: true,
        items: [
            { title: 'AI GOV - Gestão de Projetos', icon: Monitor },
            { title: 'AI KPI - Monitoramento SGIA', icon: BarChart3 },
        ],
        footer: { icon: Zap, text: 'ALGOR Cloud Active', pulse: true }
    }
];
                ...
                >
                    {offerings.map((offering, idx) => (
                        <motion.div
                            ...
                            className={`group relative ${offering.featured ? 'lg:scale-105' : ''}`}
                        >
                            {/* Gradient Border Effect */}
                            <div
                                className={`absolute inset-0 rounded-2xl p-[1px] bg-gradient-to-b ${offering.gradient} opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
                            />

                            {/* Card Content */}
                            <div className="relative h-full bg-gradient-to-b from-[#131825]/90 to-[#0A0E1A]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-8 group-hover:border-transparent transition-all overflow-hidden">
                                ...
                                <div className="flex items-center gap-4 mb-8 relative z-10">
                                    ...
                                    >
                                        <div
                                            ...
                                            style={{ backgroundColor: `${offering.color}30` }}
                                        />
                                        <div
                                ...
                                <div className="space-y-4 relative z-10">
                                    {offering.items.map((item, itemIdx) => (
                                        ...
                                        >
                                            ...
                                            <div>
                                                <h4 className="text-white font-medium text-sm group-hover/item:text-white transition-colors">
                                                    {item.title}
                                                </h4>
                                            </div>
                                        </motion.div>
