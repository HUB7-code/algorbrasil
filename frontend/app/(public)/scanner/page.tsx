...
export default function ScannerFreemiumPage() {
    ...
    const [scanMode, setScanMode] = useState<'upload' | 'api'>('upload');
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (!token) {
                // Redireciona para login se não estiver logado (Persona A Requirement: Cadastro para usar)
            ...
            if (!response.ok) {
                ...
                if (response.status === 402) {
                    // Payment Required -> Créditos acabaram
                    setError("SEUS CRÉDITOS ACABARAM. Para continuar analisando, fale com um especialista ou aguarde a renovação.");
            ...
            // Force refresh das organizações para atualizar o badge de créditos no Header
            // Pequeno hack de evento customizado ou reload suave
            ...
            <div className="relative z-10 max-w-4xl mx-auto px-6">
                ...
                <div className="flex justify-center mb-8">
                    <div className="bg-white/5 p-1 rounded-xl flex items-center">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${scanMode === 'upload'
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all flex items-center gap-2 ${scanMode === 'api'
                ...
                {scanMode === 'api' ? (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        className="bg-[#0D1117] border border-gray-800 rounded-2xl p-6 relative overflow-hidden"
                    >
                        <div className="flex items-center justify-between mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 className="text-white font-bold">Integration Mode</h3>
                                    <p className="text-gray-400 text-xs">Teste a API diretamente do seu terminal</p>
                                </div>
                            </div>
                        ...
                        <div className="bg-black/50 rounded-xl p-4 font-mono text-sm text-gray-300 overflow-x-auto border border-gray-800 relative group">
                            <button
                                onClick={() => navigator.clipboard.writeText(`curl -X POST ${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/scanner/upload \\\n  -H "Authorization: Bearer ${localStorage.getItem('algor_token') || 'SEU_TOKEN'}" \\\n  -F "file=@dados.csv"`)}
                            ...
                            <span className="text-purple-400">curl</span> -X POST {process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000/api/v1'}/scanner/upload \<br />
                            &nbsp;&nbsp;-H <span className="text-green-400">"Authorization: Bearer {localStorage.getItem('algor_token')?.slice(0, 10)}..."</span> \<br />
                            &nbsp;&nbsp;-F <span className="text-yellow-400">"file=@dados.csv"</span>
                        </div>
                        ...
                        <div className="mt-6 flex gap-4">
                            <div className="flex-1 p-4 bg-white/5 rounded-xl border border-white/5">
                                ...
                                <p className="text-gray-400 text-xs leading-relaxed">
                                    Integre o ALGOR Scanner no seu GitHub Actions ou Jenkins para bloquear deploys com vazamento de dados.
                                </p>
                            </div>
                            <div className="flex-1 p-4 bg-white/5 rounded-xl border border-white/5">
                                <h4 className="text-white text-sm font-medium mb-2 flex items-center gap-2">
                                    <span className="material-symbols-rounded text-green-400 text-sm">security</span>
                                    Pre-Commit Hook
                                </h4>
                                <p className="text-gray-400 text-xs leading-relaxed">
                                    Use nossa CLI para sanitizar dados localmente antes mesmo do commit. (Enterprise Only)
                                </p>
                            </div>
                        </div>
                    </motion.div>
