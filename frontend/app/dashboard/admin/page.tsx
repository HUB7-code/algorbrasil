    ...
    const handleCreateInvite = async (e: React.FormEvent) => {
        ...
        try {
            const res = await fetch("/api/v1/admin/invite", {
                ...
                body: JSON.stringify({ client_email: inviteEmail })
            });
            const data = await res.json();
            if (res.ok) setInviteLink(data.invite_link);
        } catch (err) {
                    ...
                    {activeTab === 'lms' && (
                        <div className="space-y-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {/* Create Card */}
                                <div className="border border-dashed border-white/10 rounded-3xl bg-white/5 flex flex-col items-center justify-center p-8 text-center hover:bg-white/10 transition-colors cursor-pointer group">
                                ...
                                {courses.map(course => (
                                    <div key={course.id} className="glass-panel p-6 rounded-3xl border border-white/5 relative group">
                                        <div className="flex justify-between items-start mb-4">
                                            ...
                                            <button onClick={() => handleDeleteCourse(course.id)} className="text-gray-500 hover:text-red-400 transition-colors">
                                                <span className="material-symbols-rounded text-lg">delete</span>
                                            </button>
                                        </div>
                                        <h3 className="text-lg font-bold text-white mb-2">{course.title}</h3>
                                        <p className="text-xs text-gray-400 line-clamp-2">Gerenciamento via API ou Dashboard.</p>
                                    </div>
                                ))}
                            </div>
                        </div>
